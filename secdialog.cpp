...
QVector<double> SecDialog::Statistique()
{
    ...
    stat[3]=0;
    stat[4]=0;
    q.prepare("SELECT QUANTITE_INGREDIENT FROM PRODUIT WHERE QUANTITE_INGREDIENT<=5");
    q.exec();
    while (q.next())
    {
        stat[0]++;
    }
    q.prepare("SELECT QUANTITE_INGREDIENT FROM PRODUIT WHERE QUANTITE_INGREDIENT>5 and QUANTITE_INGREDIENT<=15");
    q.exec();
    while (q.next())
    {
        stat[1]++;
    }
    q.prepare("SELECT QUANTITE_INGREDIENT FROM PRODUIT WHERE QUANTITE_INGREDIENT>15 and QUANTITE_INGREDIENT<=25");
    q.exec();
    while (q.next())
    {
        stat[2]++;
    }
    q.prepare("SELECT QUANTITE_INGREDIENT FROM PRODUIT WHERE QUANTITE_INGREDIENT>25 and QUANTITE_INGREDIENT<=35");
    q.exec();
    while (q.next())
    {
        stat[3]++;
    }
    q.prepare("SELECT QUANTITE_INGREDIENT FROM PRODUIT WHERE QUANTITE_INGREDIENT>35 ");
    q.exec();
    while (q.next())
    {
        stat[4]++;
